...
def get_ochl_volume_data():
    ...
    stocks_df = pd.read_csv('/Users/thomasario/Coding/AI/Low_float_ground_up/stocks.csv')
    ...
    for i in range(0, len(stocks_df), 50):
        ...
        for Ticker in stocks_df['Ticker'][i:i+50]:
            ochl_volume_data = finnhub_client.stock_candles(Ticker, 'D', int(pd.Timestamp("now").timestamp())-31536000, int(pd.Timestamp("now").timestamp()))
    ...
    for Ticker in stocks_df['Ticker']:
        ...
        stock_ochl_volume_rsi_vwap_df['Price % Change'] = stock_ochl_volume_rsi_vwap_df['c'].pct_change(periods=2)
